...
export class IdentityDocumentService extends HelperBaseService {
  ...
  addIdentityDocument(
    ...
    const obj = {
      ...
      KycProfile:
        KycProfile != null && KycProfile.length > 0 ? KycProfile : null,
  ...
  saveHelper(): Observable<any> {
    const uri_ =
      this.apiUrl + "/IdentityDocument/update/" + this.identityDocument;
  ...
  //********************************************************************
  loadHelper(id) {
    this.getIdentityDocument(id).subscribe((res: IdentityDocument) => {
