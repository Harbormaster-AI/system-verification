...
def get(request):
    ...
    exchange_rate_id = request_data["id"]
    delegate = ExchangeRateDelegate()
    request_data = delegate.get(exchange_rate_id)
    as_json = serializers.serialize("json",request_data)
    return HttpResponse(as_json,content_type="application/json")

def create(request):
	exchange_rate = json.loads(request.body)
	delegate = ExchangeRateDelegate()
	request_data = delegate.createFromJson( exchange_rate )
	as_json = serializers.serialize("json", request_data)
	return HttpResponse(as_json, content_type="application/json");
...
def update(request):
	exchange_rate = json.loads(request.body)
	delegate = ExchangeRateDelegate()
	request_data = delegate.save( exchange_rate )
	as_json = serializers.serialize("json", request_data)
	return HttpResponse(as_json, content_type="application/json");
...
def delete(request):
    ...
    exchange_rate_id = request_data["id"]
    delegate = ExchangeRateDelegate()
    request_data = delegate.delete(exchange_rate_id)
    as_json = serializers.serialize("json",request_data)
    return HttpResponse(as_json,content_type="application/json")
