...
class KycProfileDelegate:
    ...
    def get(self, kyc_profile_id):
        ...
        try:
            ...
            raise Exceptions.ProcessingError(
                "KycProfile with id " + str(kyc_profile_id) + " does not exist."
            )
    ...
    def addIdentityDocuments(self, kyc_profile_id, identity_documents_ids):
        # lazy importing avoids circular dependencies
        from bankingOnDjango.delegates.IdentityDocumentDelegate import (
            IdentityDocumentDelegate,
        )

        err_msg = (
            "Failed to add elements "
            + str(identity_documents_ids)
            + " for IdentityDocuments on KycProfile"
        )
        ...
        try:
            ...
            raise Exceptions.ProcessingError(
                err_msg + " : IdentityDocument does not exist."
            )
        except Exception:
    ...
    def addRiskAssessments(self, kyc_profile_id, risk_assessments_ids):
        # lazy importing avoids circular dependencies
        from bankingOnDjango.delegates.RiskAssessmentDelegate import (
            RiskAssessmentDelegate,
        )

        err_msg = (
            "Failed to add elements "
            + str(risk_assessments_ids)
            + " for RiskAssessments on KycProfile"
        )
        ...
        try:
            ...
            raise Exceptions.ProcessingError(
                err_msg + " : RiskAssessment does not exist."
            )
        except Exception:
            raise Exceptions.ProcessingError(err_msg)
    ...
    def addScreenings(self, kyc_profile_id, screenings_ids):
        # lazy importing avoids circular dependencies
        from bankingOnDjango.delegates.ScreeningResultDelegate import (
            ScreeningResultDelegate,
        )

        err_msg = (
            "Failed to add elements "
            + str(screenings_ids)
            + " for Screenings on KycProfile"
        )
        ...
        try:
            ...
            raise Exceptions.ProcessingError(
                err_msg + " : ScreeningResult does not exist."
            )
        except Exception:
            raise Exceptions.ProcessingError(err_msg)
    ...
    def removeScreenings(self, kyc_profile_id, screenings_ids):
        ...
        try:
            ...
            raise Exceptions.ProcessingError(
                "ScreeningResult with id " + str(screenings_id) + " does not exist."
            )
        except utils.Exceptions.DatabaseError:
            raise Exceptions.StorageWriteError()
        except Exception:
