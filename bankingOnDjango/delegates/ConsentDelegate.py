...
class ConsentDelegate:
    ...
    def unassignCustomer(self, consent_id):
        err_msg = (
            "Failed to unassign element " + str(consent_id) + " for Customer on Consent"
        )
    ...
    def unassignBank(self, consent_id):
        err_msg = (
            "Failed to unassign element " + str(consent_id) + " for Bank on Consent"
        )
    ...
    def removeAuthorizedAccounts(self, consent_id, authorized_accounts_ids):
        ...
        try:
            ...
            raise Exceptions.ProcessingError(
                "Account with id " + str(authorized_accounts_ids) + " does not exist."
            )
        except utils.Exceptions.DatabaseError:
