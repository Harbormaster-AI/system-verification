...
class StandingInstructionDelegate:
    ...
    def get(self, standing_instruction_id):
        err_msg = "Failed to get StandingInstruction from db using id " + str(
            standing_instruction_id
        )
        try:
            standing_instruction = StandingInstruction.objects.filter(
    ...
    def delete(self, standing_instruction_id):
        err_msg = "Failed to delete StandingInstruction from db using id " + str(
            standing_instruction_id
        )

        try:
            standing_instruction = StandingInstruction.objects.get(
                id=standing_instruction_id
            )
    ...
    def getAll(self):
        try:
            ...
            raise Exceptions.StorageReadError(
                "Failed to get all StandingInstruction from db"
            )
        except Exception:
            return None
    ...
    def assignAccount(self, standing_instruction_id, account_id):
        ...
        err_msg = (
            ...
            + str(account_id)
            + " for Account on StandingInstruction"
        )
    ...
    def assignBeneficiary(self, standing_instruction_id, beneficiary_id):
        ...
        err_msg = (
            "Failed to assign element "
            + str(beneficiary_id)
            + " for Beneficiary on StandingInstruction"
        )
