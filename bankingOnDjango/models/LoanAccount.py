...
class LoanAccount(models.Model):
    ...
    repayment_schedule = models.ManyToManyField(
        "RepaymentSchedule", blank=True, related_name="+"
    )
    ...
    loan_type = models.CharField(
        max_length=64, null=True, choices=[(tag.name, tag.value) for tag in LoanType]
    )
    rate_type = models.CharField(
        max_length=64, null=True, choices=[(tag.name, tag.value) for tag in RateType]
    )
    compounding = models.CharField(
        max_length=64,
        null=True,
        choices=[(tag.name, tag.value) for tag in InterestCompounding],
    )
    status = models.CharField(
        max_length=64, null=True, choices=[(tag.name, tag.value) for tag in LoanStatus]
    )
