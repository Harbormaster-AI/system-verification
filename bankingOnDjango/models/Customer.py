...
#======================================================================
	...
	external_accounts = models.ManyToManyField('ExternalAccount',  blank=True, related_name='+')
	funds_transfers = models.ManyToManyField('FundsTransfer',  blank=True, related_name='+')
	...
	customer_type = models.CharField(max_length=64, null=True, choices=[(tag.name, tag.value) for tag in CustomerType])
	risk_rating = models.CharField(max_length=64, null=True, choices=[(tag.name, tag.value) for tag in RiskRating])
	kyc_status = models.CharField(max_length=64, null=True, choices=[(tag.name, tag.value) for tag in KycStatus])
