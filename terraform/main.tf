...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "cdfbb111-58e3-4b6e-b016-9db71d4caadc"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.14929826415045144"
...
#  name        = "crmonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application crmonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for crmonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "crmonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "crmonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_crmonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "crmonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/crm-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/crmonaspdotnet theharbormaster/crm-onaspdotnet:latest"
    ]
  }
}
