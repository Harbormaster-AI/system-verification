...
resource "aws_db_subnet_group" "default" {
  name       = "ecommerceonaspdotnet-db-subnet"
...
#  name        = "ecommerceonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application ecommerceonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for ecommerceonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "ecommerceonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "ecommerceonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_ecommerceonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "ecommerceonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/ecommerce-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/ecommerceonaspdotnet theharbormaster/ecommerce-onaspdotnet:latest"
    ]
  }
}
