...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "b93cba24-44ac-4948-a70a-af5f2036ce59"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.7464140876213436"
...
#  name        = "iotonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application iotonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for iotonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "iotonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "iotonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_iotonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "iotonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/iot-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/iotonaspdotnet theharbormaster/iot-onaspdotnet:latest"
    ]
  }
}
