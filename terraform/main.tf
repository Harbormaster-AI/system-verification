...
resource "aws_db_subnet_group" "default" {
  name       = "healthcareonaspdotnet-db-subnet"
...
#  name        = "healthcareonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application healthcareonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for healthcareonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "healthcareonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "healthcareonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_healthcareonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "healthcareonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/healthcare-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/healthcareonaspdotnet theharbormaster/healthcare-onaspdotnet:latest"
    ]
  }
}
