...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "5314b79f-f743-4f28-84f0-339cc05e3f37"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.16227102906458235"
...
resource "aws_db_subnet_group" "default" {
  name       = "advertisingonrails-db-subnet"
...
#  name        = "advertisingonrails-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application advertisingonrails created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for advertisingonrails and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "advertisingonrails-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "advertisingonrails"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_advertisingonrails"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "advertisingonrails instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/advertising-on-rails:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/advertisingonrails theharbormaster/advertising-on-rails:latest"
    ]
  }
}
