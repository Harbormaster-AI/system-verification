...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "9372049a-9622-4af1-a9b5-eaf98aa5d6c3"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.9180699359424007"
...
resource "aws_db_subnet_group" "default" {
  name       = "inventoryonaspdotnet-db-subnet"
...
#  name        = "inventoryonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application inventoryonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for inventoryonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "inventoryonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "inventoryonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_inventoryonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "inventoryonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/inventory-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/inventoryonaspdotnet theharbormaster/inventory-onaspdotnet:latest"
    ]
  }
}
