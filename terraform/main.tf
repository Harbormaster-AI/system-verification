...
resource "aws_db_subnet_group" "default" {
  name       = "fintechonaspdotnet-db-subnet"
...
#  name        = "fintechonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application fintechonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for fintechonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "fintechonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "fintechonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_fintechonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "fintechonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/fintech-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/fintechonaspdotnet theharbormaster/fintech-onaspdotnet:latest"
    ]
  }
}
